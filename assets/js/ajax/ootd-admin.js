$(document).ready(function() {
    ...
    $(document).on("click", ".paginate", async function() {
        ...
        let ootd_posts_parsed = await ajaxRequest("GET", "", `?skip=${skip}&limit=10`);
        let ootd_posts = JSON.parse(ootd_posts_parsed);
        $(".table-body").html("");
        for(let i = 0; i < ootd_posts.length; i++) {
            setTableContent(ootd_posts[i], "PAGINATE");
        }
    });

    async function tableLoadData(skip, limit) {
        $(".page-title").text("OOTD");
        $(".table-head").append(`
            <th>Title</th>
            <th>Category</th>
            <th>Date</th>
        ...
        let ootd_posts_parsed = await ajaxRequest("GET", "", `?skip=${skip}&limit=${limit}`);
        let ootd_posts = JSON.parse(ootd_posts_parsed);
        for(let i = 0; i < ootd_posts.length; i++) {
            setTableContent(ootd_posts[i], "PAGINATE");
        }

        let ootd_posts_count = await ajaxRequest("GET", "", `?count=true`);
        let result = Math.ceil(ootd_posts_count / 10);
        let j = 0;
        while(j<result) {
